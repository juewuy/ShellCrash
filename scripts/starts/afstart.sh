...
if [ -n "$test" -o -n "$(pidof CrashCore)" ]; then
    ...
    #加载定时任务
    #清理旧版守护残条(/bin/ash),重新生成 /bin/sh 版避免堆积
    [ -f "$TASKCFGDIR"/cron ] && sed -i '/start_legacy_wd\.sh/d;/bot_tg\.sh/d' "$TASKCFGDIR"/cron 2>/dev/null
    [ -f "$TASKCFGDIR"/running ] && sed -i '/start_legacy_wd\.sh/d;/bot_tg\.sh/d' "$TASKCFGDIR"/running 2>/dev/null
    cronload | grep -vE '^$|start_legacy_wd\.sh|bot_tg\.sh' >"$TMPDIR"/cron_tmp
